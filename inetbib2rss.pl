...
$log->debugf("Fetching '%s'",$InetBibArchiv);
...
$log->debugf("Adding item %d: %s: \"%s\" (%s)",$i,$author,$subject,$date);
            $Rss->add_item(
                    title       => $subject,
                    link        => $url,
                    description => $desc,
    ...
    open (F,">",$outfile)
     or die_with_error(sprintf("Kann '%s' nicht oeffnen: %s",$outfile,$!));
    print F $Rss->as_string();
    close(F) or die_with_error(sprintf("Kann '%s' nicht schliessen: %s",$outfile,$!));
    $log->infof("created '%s' successfully",$outfile);
} ## end if ( $response->{success...})
else {
  $error = sprintf("ERROR: Konnte URL $InetBibArchiv nicht abrufen: %s (%s)",$InetBibArchiv,$response->{reason},$response->{status});
  $log->error($error);
    die $error;
}
...
$log->infof("Started at '%s'...", DateTime->now->strftime("%d.%m.%Y %H:%M:%S"));
GetOptions('help|?' => \$help, "outfile=s" => \$outfile) or pod2usage(2);
pod2usage(1) if $help;

die_with_error("Kein Parameter 'outfile' uebergeben!") unless (defined($outfile));
...
./inetbib2rss.pl -o=/var/www/rss/inetbib.xml

./inetbib2rss.pl --outfile=/var/www/rss/inetbib.xml
